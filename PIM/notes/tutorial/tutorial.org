...
TODO réexpliquer ça mieux :

The visibility can change (put on the first heading to test the cycle), like that :

#+begin_example

Folded -> Press TAB -> View only subheading -> Press TAB -> View all the text -> Press TAB -> Folded -> …

#+end_example

Now that you understand this, how can you access of all the magic ? In the Menu Bar of course !
TODO meilleur explication de ça.
...
The package to do all these things is "Org".
...
You can click elsewhere (or use =ESC=) to remove the mark.
...
Furthermore, there will be a lot of shortcuts in this section that will not (always) be in the menu (because sometimes, the default shortcut of Emacs are good !) but don't forget :
=You also rely on the Menu Bar, the Org Submenu, or even M-x= (if you want to just try a command for example).
To see all methods to call commands, you can click [[id:657dd016-284e-40ad-8591-4b3cac5f3cc7][here]].

For the following, you can fold and jump to the part that interest you the most : =You don't have to read this in a linear way.=
...
If you want change the shortcut to open the Menu, todo customize the variable "eepim-master-hydra".

As you can see in the menu, on the right, you have a section "Nodes", and on the left, you will have other "submenus".
...
But… where is these famous IDs ? In this tutorial (and only here), there are hidden. To view the IDs, you can call the command "org-tidy-mode".
As you can see, the IDs are in :Properties: drawers, just under the heading : you can fold/expand the drawer like a heading. But in general, it is not really useful.
...
If you, call the command "org-tidy-mode". The drawer will be "hidden" of your view, but it is still here : it is marked by the "#" at the left side. As you can see in this tutorial, the drawer are hidden.
If you want to always hide the drawer elsewhere, todo customize the variable "eepim-org-tidy".
...
To finish, when you are searching a node in the Minibuffer, you can have a preview of the node (use the arrow (down and up) to switch the preview).
...
TODO mieux expliquer, et juste dire qu'il peut modifier. Trop complexe, cacher la syntaxe ?
todo créer un node
...
Don't pay attention to the "Properties" drawer (and don't modify it) : it is just here because you are actually creating a node (see todo partie avancée)

You can write what you want, and then finish the "capture" process. (todo décrire capture !)

To have your own template (todo pré configuration), you can view this : [[id:29254ea8-09b8-4456-98c8-690433b84bca][Exemple 2 : Org-roam templates]].
...
Maybe you don't want to be annoyed with nodes of the Tutorial.
To deactivate all the search in the Tutorial, you can customize (todo) the variable "eepim-include-tutorial".
...
However, for more details about all the concept that Org mode can handle, you can consult this section todo.
...
- e : export your agenda into another format (todo)
